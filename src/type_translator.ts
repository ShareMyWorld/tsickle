...
export class TypeTranslator {
  ...
  private translateUnion(type: ts.UnionType, typeNode: ts.UnionTypeNode|undefined): string {
    // TypeScript expands type aliases in union members, which drops the distinction in between a
    // union of a type alias and some member (`StringOrNumber|boolean`), and a union of the members
    // of that type alias (`string|number|boolean`). That's a problem if the constituent types are
    // not valid symbols in the lexical context (e.g. not imported).
    // The code below solves the problem by emitting union types from their syntactical
    // representation (`typeNode`) rather than their semantic representation (`type`) if available.
    let parts: string[];
